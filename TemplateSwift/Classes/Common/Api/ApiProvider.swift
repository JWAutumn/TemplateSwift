...
extension ApiProvider {
    ...
    public static func defaultEndpointClosure(target: Target) -> Endpoint {
        ...
        if let token = UserDefaults.LoginInfo.value(forKey: .token) as? String,
        ...
        let plugin = NetworkActivityPlugin { (changeType, target) in
            ...
            case.began:
                DispatchQueue.main.async {
                    ...
                    case .normal: HUD.show(view: view)
                    case .opaque: HUD.show(view: view, text: nil, opaque: true, offset: .zero)
                    case .text(let text): HUD.show(view: view, text: text, opaque: false, offset: .zero)
                    case .none: break
                    }
                }
            case .ended:
